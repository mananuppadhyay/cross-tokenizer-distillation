...
#Taken from https://github.com/Nicolas-BZRD/llm-recipes/blob/main/models/distillation_model.py
class DistillationLoss(nn.Module):
    """
    Custom loss function for knowledge distillation.
    
    Args:
        crossentropy_weight: Weight for cross-entropy loss
        distillation_weight: Weight for distillation loss
        student_temperature: Temperature for student model softmax
        teacher_temperature: Temperature for teacher model softmax
        skip_student_eos: Whether to skip student's EOS token
        skip_teacher_eos: Whether to skip teacher's EOS token
    ...
    def forward(self, 
        ...
        Args:
            student_predictions: Output from student model
            teacher_predictions: Output from teacher model
            student_targets: Target labels for student
            teacher_targets: Target labels for teacher
            
        Returns:
            tuple: Total loss, cross-entropy loss, and distillation loss
        """
    ...
    def __get_start_and_size_answers(self, answer_tensors):
        """
        Extract the start indices and sizes of answers from tensors.
        
        Args:
            answer_tensors: Tensor containing answer token IDs
            
        Returns:
            tuple: Lists of start indices and sizes
        """
        answers_index = []
        answers_size = []
